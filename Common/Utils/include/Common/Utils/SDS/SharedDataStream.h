#ifndef DEVICE_CLIENT_SDK_COMMON_UTILS_SDS_SHAREDDATASTREAM_H_
#define DEVICE_CLIENT_SDK_COMMON_UTILS_SDS_SHAREDDATASTREAM_H_
...
size_t SharedDataStream<T>::calculateBufferSize(size_t nWords, size_t wordSize, size_t maxReaders) {
    if (0 == nWords) {
        LOG_ERROR << "calculateBufferSizeFailed; reason: numWordsZero";
        return 0;
    } else if (0 == wordSize) {
        LOG_ERROR << "calculateBufferSizeFailed; reason: wordSizeZero";
        return 0;
    }
...
std::unique_ptr<SharedDataStream<T>> SharedDataStream<T>::create(
    ...
    if (0 == expectedSize) {
        ...
        LOG_ERROR << "createFailed; reason: nullBuffer";
        return nullptr;
    } else if (expectedSize > buffer->size()) {
        LOG_ERROR << "createFailed; reason: bufferSizeTooSmall";
        return nullptr;
    }
...
std::unique_ptr<typename SharedDataStream<T>::Writer> SharedDataStream<T>::createWriter(
    ...
    if (header->isWriterEnabled && !forceReplacement) {
        LOG_ERROR << "createWriterFailed; reason: existingWriterAttached";
        return nullptr;
    } else {
...
std::unique_ptr<typename SharedDataStream<T>::Reader> SharedDataStream<T>::createReader(
    ...
    LOG_ERROR << "createWriterFailed; reason: noAvailableReaders";
    return nullptr;
}
...
std::unique_ptr<typename SharedDataStream<T>::Reader> SharedDataStream<T>::createReaderLocked(
    ...
    if (m_bufferLayout->isReaderEnabled(id) && !forceReplacement) {
        LOG_ERROR << "createReaderLockedFailed; reason: readerAlreadyAttached";
...
#endif // DEVICE_CLIENT_SDK_COMMON_UTILS_SDS_SHAREDDATASTREAM_H_
