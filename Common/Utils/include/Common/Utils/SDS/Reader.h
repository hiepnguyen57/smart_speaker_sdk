#ifndef DEVICE_CLIENT_SDK_COMMON_UTILS_SDS_READER_H_
#define DEVICE_CLIENT_SDK_COMMON_UTILS_SDS_READER_H_
...
ssize_t SharedDataStream<T>::Reader::read(void* buf, size_t nWords, std::chrono::milliseconds timeout) {
    if (nullptr == buf) {
        LOG_ERROR << "readFailed; reason: nullBuffer";
        return Error::INVALID;
    }

    if (0 == nWords) {
        LOG_ERROR << "readFailed; reason: invalidNumWords";
        return Error::INVALID;
    }
...
bool SharedDataStream<T>::Reader::seek(Index offset, Reference reference) {
    ...
    switch (reference) {
        ...
        case Reference::BEFORE_READER:
            if (offset > *m_readerCursor) {
                LOG_ERROR << "seekFailed; reason: seekBeforeStreamStart; referent: BEFORE_READER";
                return false;
            }
            absolute = *m_readerCursor - offset;
            break;
        case Reference::BEFORE_WRITER:
            if (offset > *writeStartCursor) {
                LOG_ERROR << "seekFailed; reason: seekBeforeStreamStart; referent: BEFORE_WRITER";
                return false;
            }
    ...
    if (absolute > *m_readerCloseIndex) {
        LOG_ERROR << "seekFailed; reason: seekBeyondCloseIndex";
        return false;
    }
    ...
    if (*writeEndCursor >= absolute && *writeEndCursor - absolute > m_bufferLayout->getDataSize()) {
        LOG_ERROR << "seekFailed; reason: seekOverwrittenData";
        return false;
    }
...
typename SharedDataStream<T>::Index SharedDataStream<T>::Reader::tell(Reference reference) const {
    ...
    LOG_ERROR << "tellFailed; reason: invalidReference";
...
void SharedDataStream<T>::Reader::close(Index offset, Reference reference) {
    ...
    switch (reference) {
        ...
        case Reference::BEFORE_WRITER:
            if (*writeStartCursor < offset) {
                LOG_ERROR << "closeFailed; reason: invalidIndex; referent: BEFORE_WRITER";
    ...
    if (!validReference) {
        LOG_ERROR << "closeFailed; reason: invalidReference";
    }
...
std::string SharedDataStream<T>::Reader::errorToString(Error error) {
    ...
    LOG_ERROR << "errorToStringFailed; reason: invalidError";
...
#endif // DEVICE_CLIENT_SDK_COMMON_UTILS_SDS_READER_H_
