#ifndef DEVICE_CLIENT_SDK_COMMON_UTILS_SDS_WRITER_H_
#define DEVICE_CLIENT_SDK_COMMON_UTILS_SDS_WRITER_H_
...
ssize_t SharedDataStream<T>::Writer::write(const void* buf, size_t nWords, std::chrono::milliseconds timeout) {
    if (nullptr == buf) {
        LOG_ERROR << "writeFailed; reason: nullBuffer";
        return Error::INVALID;
    }
    if (0 == nWords) {
        LOG_ERROR << "writeFailed; reason: zeroNumWords";
        return Error::INVALID;
    }

    auto header = m_bufferLayout->getHeader();
    if (!header->isWriterEnabled) {
        LOG_ERROR << "writeFailed; reason: writerDisabled";
        return Error::CLOSED;
    }
...
std::string SharedDataStream<T>::Writer::errorToString(Error error) {
    ...
    LOG_ERROR << "writeFailed; reason: invalidError";
...
#endif // DEVICE_CLIENT_SDK_COMMON_UTILS_SDS_WRITER_H_
